...
namespace Drupal\webform_login\Plugin\WebformHandler;
...
class UserRegistrationWebformHandler extends WebformHandlerBase {
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    ...
    if (isset($account)) {
      ...
      if ($result === SAVED_NEW) {
        \Drupal::logger('new saved')->notice(json_encode($account->toArray()));
        $message = '';
        ...
        \Drupal::logger('account')->notice(json_encode($account->toArray()));
        user_login_finalize($account);
      }
    }
  }
